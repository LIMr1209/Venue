...
namespace DefaultNamespace
{
    ...
    public class TestRequest : MonoBehaviour
    {
        ...
        {
            UnityWebRequest
                request = UnityWebRequest.Get("http://render-dev.d3ingo.com/api/element/list"); // 获取文本或者二进制数据
            ...
            else
            {
                ...
                result = JsonUtility.FromJson<Result>(text); // ToJson
                // Debug.Log(result.meta.status_code + " " + result.meta.message);
                // foreach (Data i in result.data)
                // {
                //     Debug.Log(i.name + " " + i.mark);
                // }
